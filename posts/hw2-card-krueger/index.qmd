...
ck <- ck_raw |>
  ...
  mutate(
    ...
    fte2 = case_when(
      status2 == 2 ~ 0,
      status2 == 3 ~ empft2 + 0.5 * emppt2 + nmgrs2,
      TRUE         ~ NA_real_
    ),
    dfte = fte2 - fte1
  )
...
**Handling of closed restaurants.** Following Card & Krueger's Table 3 Column (i) specification, permanently closed stores (`status2 == 2`) are assigned FTE = 0 in wave 2, reflecting that they contribute zero employment after closure. Stores that could not be reached or refused to participate (`status2` neither 3 nor 2) are treated as missing and excluded from the balanced panel. This is the most conservative approach and the one the paper uses for its headline DiD estimate.
...
The key cell is the bottom-right: the difference-in-differences estimate of `r sprintf("%.2f", did)` FTE workers per store. Reading down the NJ − PA column:
...
- The change in NJ exceeded the change in PA by `r sprintf("%.2f", did)` FTE workers — the DiD estimate. The paper reports 2.76.
...
The NJ coefficient of `r round(coef(mod)[2], 2)` matches the paper's 2.76 (paper SE = 1.07).
