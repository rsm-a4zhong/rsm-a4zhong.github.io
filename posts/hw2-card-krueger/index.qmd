...
The replication recovers the paper's central finding with minimal deviation: ~2.76 FTE workers per store, matching the published value.

Minor discrepancies arise from sample definition (the paper has multiple cuts) and standard error choices. HC1 robust SEs are used throughout, as in the paper.

The deeper lesson is methodological. By exploiting New Jersey's minimum wage increase as a natural experiment, Card and Krueger achieved credible causal identification: DiD nets out time-invariant store differences and common industry trends, leaving only the differential change attributable to the policy. The wage distribution plot validates the key prerequisite — the treatment actually occurred — completing the causal argument.
