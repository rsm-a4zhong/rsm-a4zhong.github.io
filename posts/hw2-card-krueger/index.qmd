...
ck <- ck_raw |>
  ...
  mutate(
    ...
    fte1 = replace_na(empft, 0) + 0.5 * replace_na(emppt, 0) + replace_na(nmgrs, 0),
    fte2 = case_when(
      status2 %in% c(2, 3) ~ 0,                             # closed/renovating → FTE = 0
      !is.na(empft2)        ~ replace_na(empft2, 0) + 0.5 * replace_na(emppt2, 0) + replace_na(nmgrs2, 0), # open & interviewed → compute FTE
